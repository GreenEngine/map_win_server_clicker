...
| `MCP_UPDATE_USE_PS1` | На **Windows** при старте **`server.py`**, если переменная **не задана**, подставляется **`1`**: **`server_update`** вызывает PowerShell [scripts/update_server.ps1](scripts/update_server.ps1) (git pull + pip в venv). После успеха при **`MCP_RESTART_AFTER_UPDATE=1`** скрипт через **`Start-Process`** запускает [scripts/mcp_restart_after_update.py](scripts/mcp_restart_after_update.py), родительский MCP завершается (~2.5 с). Отключить PS1-путь: **`0`** / **`false`** / **`no`**. |
