...
| `server_update` | `pip` / `git_pull` / `full`; лог в `data.log`; автоперезапуск только при реальных изменениях git/pip (см. **`MCP_RESTART_AFTER_UPDATE_ALWAYS`**). Нужен **`MCP_ALLOW_SELF_UPDATE=1`**; иначе `ERR_FORBIDDEN`. |
