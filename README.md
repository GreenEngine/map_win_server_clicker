...
| `MCP_RESTART_AFTER_UPDATE` | **`1`** / **`true`** (по умолчанию) — при необходимости запланировать автоперезапуск MCP после **`server_update`**. Если **git** и **pip** не меняли рабочее дерево/пакеты (типичный вывод *Already up to date* и только *Requirement already satisfied*), перезапуск **не планируется** — меньше обрывов сессии. Принудительно перезапускать всегда: **`MCP_RESTART_AFTER_UPDATE_ALWAYS=1`**. Helper [`scripts/mcp_restart_after_update.py`](scripts/mcp_restart_after_update.py): ждёт PID, **ждёт освобождения порта**, до **3** попыток `Popen`; логи — **`logs/mcp_restart_helper.log`**, stderr нового процесса — **`logs/mcp_server_stderr.log`**. Полностью отключить автоперезапуск: **`0`**, **`false`**, **`no`**. |
...
| `MCP_UPDATE_USE_PS1` | На **Windows** при старте **`server.py`**, если переменная **не задана**, подставляется **`1`**: **`server_update`** для **`pip` / `git_pull` / `full`** вызывает [scripts/update_server.ps1](scripts/update_server.ps1) **с `-SkipGit`** (только **pip** в venv с локальным кэшем). Режимы **`git_pull`** и **`full`** сначала делают **git** в Python (**`git fetch origin`** + **`git pull --ff-only origin/<ветка>`** по текущей `HEAD` и fallback `main`/`master` на `origin`), затем PS1 — чтобы автоподтягивание не зависело от «голого» `git pull` в PS1. Перезапуск MCP — в `update.schedule_restart_after_update`. Отключить PS1-путь: **`0`** / **`false`** / **`no`** (тогда pip тоже через **`python -m pip`** в текущем интерпретаторе). |
| `MCP_UPDATE_SYNC` | **`1`** / **`true`** / **`yes`** — **`server_update`** ждёт конца git/pip в том же запросе (долго, клиент может оборвать таймаут). Иначе по умолчанию обновление **в фоновом потоке**: ответ сразу, полный вывод в **`logs/mcp_self_update.log`**, перезапуск после успеха как раньше. Во время прогона в файл и **по умолчанию в stderr** пишутся строки **`[SELF_UPDATE]`** с UTC (git fetch/pull, pip) — в консоли рядом с логами Uvicorn видно, что именно обновляется. Файл: `Get-Content ...\logs\mcp_self_update.log -Wait -Tail 30`. |
| `MCP_SELF_UPDATE_TRACE_STDERR` | По умолчанию **`[SELF_UPDATE]`** дублируется в **stderr** (консоль). Чтобы **не** засорять консоль: **`0`** / **`false`** / **`no`**. |
...
1. **Код** — клонируй **[map_win_server_clicker](https://github.com/GreenEngine/map_win_server_clicker)** на Windows; **`server_update`** с **`full`** делает **`git fetch` + `git pull --ff-only origin/<ветка>`** (Python) **+ pip** (через PS1 в venv при **`MCP_UPDATE_USE_PS1=1`**). Подробно: **[docs/GIT_SETUP.md](docs/GIT_SETUP.md)**. Копирование папки без **`.git`** — только ручные обновления.
2. **Зависимости и git из агента:** при **`MCP_ALLOW_SELF_UPDATE=1`** вызовите инструмент **`server_update`**. **По умолчанию режим `full`** (git fetch + pull --ff-only на `origin/<ветка>` + pip) — без аргумента всегда полный цикл, как ожидают агенты после merge. Явно **`mode=pip`** — только pip без `git pull`; **`git_pull`** — то же по git+pip, что и **`full`**. По умолчанию **`data.update_async`: true** — git/pip идут **в фоне**, HTTP-ответ приходит сразу; полный лог и факт перезапуска: **`logs/mcp_self_update.log`**. При **`MCP_RESTART_AFTER_UPDATE=1`** перезапуск через helper **только если** git/pip что-то меняли; иначе процесс **не завершается**. В быстром ответе **`data.restart_scheduled`** обычно **`false`** (перезапуск решается в фоне). Ждать конца обновления в одном запросе: **`MCP_UPDATE_SYNC=1`**. Всегда перезапускать после update: **`MCP_RESTART_AFTER_UPDATE_ALWAYS=1`**. Если **`MCP_RESTART_AFTER_UPDATE=0`**, после смены **`.py`** перезапустите **`server.py`** вручную.
...
| `server_update` | По умолчанию **`full`** (git+pip); явно **`pip`** / **`git_pull`** / **`full`** (синонимы полного цикла в коде: `git_full`, `all`). Лог в `data.log`; автоперезапуск только при реальных изменениях git/pip (см. **`MCP_RESTART_AFTER_UPDATE_ALWAYS`**). Нужен **`MCP_ALLOW_SELF_UPDATE=1`**; иначе `ERR_FORBIDDEN`. |
