...
- После **merge** на Mac: на ВМ через агент вызвать **`server_update`** с режимом **`git_pull`** или **`full`** при **`MCP_ALLOW_SELF_UPDATE=1`**. Ответ по умолчанию приходит сразу (**`data.update_async`: true**); полный вывод git/pip — **`logs/mcp_self_update.log`**. На Windows по умолчанию **git/pip** идут через **PowerShell** [scripts/update_server.ps1](../scripts/update_server.ps1); перезапуск планирует **Python** (`schedule_restart_after_update` → **`mcp_restart_after_update.py`**): ожидание завершения старого PID, **ожидание освобождения TCP-порта** (`MCP_PORT`), до **3** попыток запуска нового `server.py`. Дождитесь **`data.restart_scheduled`: true** и паузы **3–10 с**, затем **`health`** → **`agent_session`** (сверка `uia_tools_revision`).
