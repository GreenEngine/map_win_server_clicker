...
- После **merge** на Mac: на ВМ через агент вызвать **`server_update`** (по умолчанию уже **`full`**: git+pip) или явно **`git_pull`** / **`full`** при **`MCP_ALLOW_SELF_UPDATE=1`**. Ответ по умолчанию приходит сразу (**`data.update_async`: true**); полный вывод git/pip и был ли перезапуск — **`logs/mcp_self_update.log`**. Перезапуск процесса **только если** git/pip что-то меняли (иначе сессия не рвётся); принудительно всегда: **`MCP_RESTART_AFTER_UPDATE_ALWAYS=1`**. На Windows при **`MCP_UPDATE_USE_PS1=1`**: **git** для этих режимов выполняется в **Python** (`fetch` + `pull --ff-only origin/<ветка>`), **pip** — через **PowerShell** [scripts/update_server.ps1](../scripts/update_server.ps1) с **`-SkipGit`** (venv). Перезапуск планирует **Python** (`schedule_restart_after_update` → **`mcp_restart_after_update.py`**): ожидание завершения старого PID, **ожидание освобождения TCP-порта** (`MCP_PORT`), до **3** попыток запуска нового `server.py`. Если в логе был перезапуск — пауза **3–10 с**, затем **`health`** → **`agent_session`** (сверка `uia_tools_revision`; в **`server`** есть **`self_update_logic`** для проверки ветки логики обновления).
